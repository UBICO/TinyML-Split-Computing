function createDeviceCard(device) {
    ...
    var card = $('<div>').addClass('card').addClass("device-stats-card");
    var cardBody = $('<div>').addClass('card-body');
    var cardTitle = $('<h5>').addClass('card-title').text(' Device');
    // Prepend the SVG icon to the card title
    cardTitle.prepend(svgIcon);
    var cardText1 = $('<p>').addClass('card-text').text('Connected Devices: ' + device.connected_device_count);
    var cardText2 = $('<p>').addClass('card-text').text('Total Messages: ' + device.total_message_count);
    cardText2.append(loadingSpinner)
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.devices.length; i++) {
                ...
                $('#device-cards-container').append(card);
            }
